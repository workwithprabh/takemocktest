...
export const MOCK_TEST_INTROS: Readonly<Record<string, MockTestIntro>> = {
  ...
  'cma-final': {
    lead: 'A free CMA Final practice series covering the objective block of all ten papers. Each paper\'s Section A is 15 compulsory multiple choice questions worth 30 marks, practised here on its own timer. Every CMA Final online test here is free and returns an instant result.',
  ...
  'cma-intermediate': {
    lead: 'A free CMA Intermediate practice series covering the objective block of all eight papers. Each paper\'s Section A is 15 compulsory multiple choice questions worth 30 marks, practised here on its own timer. Every CMA Intermediate online test here is free to attempt.',
  ...
  'inet': {
    ...
    strategy: 'Every section carries its own qualifying minimum, and that is what makes INET different from a paper you can play to your strengths. English, Reasoning and Numerical Ability, General Science and Mathematical Aptitude, and General Knowledge are 25 questions and 100 marks each, and a candidate who scores heavily across three of them and falls below the minimum in the fourth does not qualify, whatever the total says. So the section you are weakest in sets your outcome, not the section you are best in, and preparation time belongs there rather than on widening a lead you cannot spend. On marking, 4 for a correct answer against 1 off for a wrong one puts break-even at one in five, so a single elimination makes attempting worthwhile.',
